      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 md:pb-6 border-b border-white/5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] md:text-xs font-bold bg-cyan-500/10 text-cyan-300 border border-cyan-500/30">
              Module 03
            </span>
            <h1 className="text-xl md:text-3xl font-extrabold text-white tracking-tight">Competency & Skill Gap Matrix</h1>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
              <div className="glass-card rounded-3xl p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-400 flex items-center justify-center font-extrabold text-xl flex-shrink-0">
                  {skillGap.current_skills?.length || 0}
                </div>
                <div>
                  <h4 className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">Current Verified Skills</h4>
                  <p className="text-xs md:text-sm font-bold text-white mt-0.5">Found in Resume</p>
                </div>
              </div>

              <div className="glass-card rounded-3xl p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-rose-500/10 text-rose-400 flex items-center justify-center font-extrabold text-xl flex-shrink-0">
                  {skillGap.gap_skills?.length || 0}
                </div>
                <div>
                  <h4 className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">High Impact Gaps</h4>
                  <p className="text-xs md:text-sm font-bold text-white mt-0.5">Missing Competencies</p>
                </div>
              </div>

              <div className="glass-card rounded-3xl p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-purple-500/10 text-purple-400 flex items-center justify-center font-extrabold text-xl flex-shrink-0">
                  {(skillGap.gap_skills?.length || 0) > 4 ? '4-6w' : '2-4w'}
                </div>
                <div>
                  <h4 className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">Estimated Upskill Time</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              {/* Required Skills */}
              <div className="glass-card rounded-3xl p-5 md:p-6 space-y-4">
                <div className="flex items-center gap-2 text-purple-400 font-bold text-sm md:text-base">
                ...
                <div className="flex flex-wrap gap-2">
                  {skillGap.required_skills?.map((skill, i) => (
                    <span
                      key={i}
                      className="px-3 py-1.5 rounded-xl text-xs font-bold bg-purple-500/10 text-purple-300 border border-purple-500/30"
                    >
                      {skill}
                    </span>
                  ))}
              ...
              <div className="glass-card rounded-3xl p-5 md:p-8 space-y-5">
                <div className="flex items-center justify-between pb-2 border-b border-white/5">
                  <div className="flex items-center gap-2 text-white font-bold text-base md:text-lg">
                    <ShieldCheck size={20} className="text-cyan-400 flex-shrink-0" />
                    <h3>Prioritized Upskill Roadmap</h3>
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono">Order of ROI</span>
                </div>
                ...
                <div className="space-y-3.5">
                  {skillGap.priority_order.map((item, i) => (
                    <motion.div
                      ...
                      className="p-4 md:p-5 rounded-2xl bg-slate-900/70 border border-white/5 hover:border-cyan-500/30 transition-all space-y-2.5"
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-2.5">
                          <span className="w-6 h-6 rounded-lg bg-cyan-500/10 text-cyan-400 font-bold text-xs flex items-center justify-center border border-cyan-500/20 flex-shrink-0">
                            #{i + 1}
                          </span>
                          <h4 className="font-bold text-white text-xs md:text-sm">{item.skill}</h4>
                        </div>
                        <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold font-mono bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 flex-shrink-0">
